...
import type { InjurySelection } from "@/lib/pi/engine";
...
type AnyKey = PhysicalInjuryKey | PsychiatricInjuryKey;
...
export default function InjurySelector({
  ...
  const allPhysical = Object.entries(physicalInjuries) as [
    PhysicalInjuryKey,
    (typeof physicalInjuries)[PhysicalInjuryKey]
  ][];
  const allPsychiatric = Object.entries(psychiatricInjuries) as [
    PsychiatricInjuryKey,
    (typeof psychiatricInjuries)[PsychiatricInjuryKey]
  ][];
  ...
  function toggleInjury(type: InjuryType, key: AnyKey, availableSeverities: Severity[]) {
  ...
  function updateInjury(
    type: InjuryType,
    key: AnyKey,
    updates: Partial<Pick<InjurySelection, "severity" | "recoveryMonths">>
  ) {
    onChange(
      injuries.map((i) =>
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        {/* Category tabs */}
        <div className="mb-5 flex gap-2 rounded-xl border border-slate-200 bg-slate-50 p-1 w-fit">
          {(["physical", "psychiatric"] as InjuryType[]).map((t) => (
            ...
            >
              {t === "physical" ? "🦴 Physical" : "🧠 Psychiatric"}
            </button>
          ))}
        </div>
